...
export default function filesReducer (state = initialState, action) {
  switch (action.type) {
    ...
    case constants.RECEIVE_SPENDING:
      return state
        .set('downloadspending', action.downloadspending)
    ...
    case constants.DOWNLOAD_FILE:
      return state.set(
        'unreadDownloads',
        state.get('unreadDownloads').add(action.file.siapath)
      )
    case constants.UPLOAD_FILE:
      return state.set(
        'unreadUploads',
        state.get('unreadUploads').add(action.siapath)
      )
    case constants.RECEIVE_FILES: {
      const workingDirectoryFiles = ls(
        action.files.concat(state.get('folders')),
        state.get('path')
      )
      const workingDirectorySiapaths = workingDirectoryFiles.map(
        file => file.siapath
      )
      // filter out selected files that are no longer in the working directory
      const selected = state
        .get('selected')
        .filter(file => workingDirectorySiapaths.includes(file.siapath))
      return state
    ...
    case constants.ADD_FOLDER: {
      ...
      return state
        ...
        .set(
          ...
          ls(state.get('files').concat(folders), state.get('path')),
          folders,
          state.get('path')
        )
    }
    case constants.DELETE_SIA_UI_FOLDER: {
      return state.set(
        'folders',
        state.get('folders').filter(folder => {
    ...
    case constants.SET_ALLOWANCE:
      return state.set('allowance', action.funds).set('settingAllowance', true)
    ...
    case constants.SET_SEARCH_TEXT: {
      ...
      return state.set('searchResults', results).set('searchText', action.text)
    }
    case constants.SET_PATH: {
      ...
      return state
        ...
        .set(
          'searchResults',
          searchFiles(allFiles(state), state.get('searchText'), action.path)
        )
    }
    case constants.DESELECT_FILE:
      return state.set(
        ...
        state
          .get('selected')
          .filter(file => file.siapath !== action.file.siapath)
      )
    ...
    case constants.SELECT_UP_TO:
      return state.set(
        ...
        rangeSelect(
          action.file,
          state.get('workingDirectoryFiles'),
          state.get('selected')
        )
      )
    ...
    case constants.RECEIVE_DOWNLOADS: {
      const untimedDownloads = action.downloads.filter(
        download =>
          Date.parse(download.starttime) > state.get('showDownloadsSince')
      )
      const previousDownloadTimes = state.get('downloadTimes')
      const downloadTimes = buildTransferTimes(
        previousDownloadTimes,
        untimedDownloads
      )
      const downloads = addTransferSpeeds(untimedDownloads, downloadTimes)
      return state
        .set('downloading', downloads)
    ...
    case constants.TOGGLE_FILE_TRANSFERS:
      return state
        .set('showFileTransfers', !state.get('showFileTransfers'))
