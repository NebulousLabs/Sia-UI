...
const spawnCommand = function(commandStr, actions) {
	...
	var commandString = commandStr.replace(/\s*\s/g, ' ').trim()
	...
	var consumeChunk = function(chunk) {
		...
		var chunkTrimmed = chunk.toString().replace(/stty: stdin isn't a terminal\n/g, '')
...
const httpCommand = function(commandStr, actions) {
	var commandString = commandStr
