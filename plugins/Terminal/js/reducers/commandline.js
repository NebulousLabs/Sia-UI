...
export default function commandLineReducer(state = initialState, action) {
	switch (action.type) {
		case constants.ADD_COMMAND:
			...
			return state
				.set('commandHistory', state.get('commandHistory').push(action.command))
				.set('commandIndex', 0)
				.set('currentCommand', '')
				.set('commandRunning', true)

		case constants.UPDATE_COMMAND: {
			// Updates output of command given by command name and id.
			const commandArray = state
				.get('commandHistory')
				.findLastEntry(
					(val) =>
						val.get('command') === action.command && val.get('id') === action.id
				)
			...
			const newCommand = command.set(
				'result',
				command.get('result') + action.dataChunk
			)
			return state.set(
				'commandHistory',
				state.get('commandHistory').set(commandIdx, newCommand)
			)
		}
		...
		case constants.END_COMMAND: {
			...
			return state
				...
				.set('commandRunning', false)
		}

		case constants.LOAD_PREV_COMMAND: {
			const newCommandIndex = Math.min(
				state.get('commandIndex') + 1,
				state.get('commandHistory').size
			)
			return state.set('commandIndex', newCommandIndex).set(
				'currentCommand',
				state
					.get('commandHistory')
					.get(
						Math.min(
							...
							state.get('commandHistory').size - 1
						)
					)
					.get('command')
			)
		...
		case constants.LOAD_NEXT_COMMAND: {
			...
			if (newCommandIndex) {
				return state.set('commandIndex', newCommandIndex).set(
					...
					state
						.get('commandHistory')
						.get(state.get('commandHistory').size - newCommandIndex)
						.get('command')
				)
			} else {
				return state
					.set('commandIndex', newCommandIndex)
					.set('currentCommand', '')
			}
		}
