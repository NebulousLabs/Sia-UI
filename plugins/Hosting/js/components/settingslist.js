...
const SettingsList = ({
	...
	const handleSettingInput = (e) => {
		if (e.target.value >= 0) {
			const settingToChange = e.target.attributes.getNamedItem('data-setting')
				.value
			actions.updateSettings(
				usersettings.map(
					(setting, key) =>
						key === settingToChange ? e.target.value : setting.get('value')
				)
			)
		}
	}

	const settingValues = (acceptcontracts, settings) =>
		settings
			.map((setting) => setting.get('value'))
			.set('acceptingContracts', acceptcontracts)
	const saveEnabled = () =>
		helper.validNumbers(usersettings.toList().toJSON()) && settingsChanged
	const resetSettings = () =>
		actions.pushSettings(
			defaultsettings.set('acceptingContracts', acceptingContracts)
		)
	...
	const HostProperties = usersettings
		.map((setting, key) => (
			<div className="property pure-g" key={key}>
				...
				<div
					className={
						'error pure-u-1-1' +
						(setting.get('value') <= Number(setting.get('min') || 0) ||
						isNaN(setting.get('value'))
							? ''
							: ' hidden')
					}
				>
					<span>
						Must be a number greater than {setting.get('min') || 'zero'}.
					</span>
				</div>
			</div>
		))
		...
		<div className="settings section">
			<div className="property row">
				...
				<div className="controls">
					<div
						className={'button' + (saveEnabled() ? '' : ' disable')}
			...
			<div className="property pure-g">
				...
				<div className="pure-u-1-2">
					<div className="value">
						<div
							className={'toggle-switch' + (acceptingContracts ? '' : ' off')}
