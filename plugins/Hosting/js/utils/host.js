...
export const validNumbers = values =>
  ...
  values.reduce(
    (isValid, val) =>
      !isNaN(val.value) && val.value > (val.min || 0) && isValid,
    true
  )
...
export const blocksToWeeks = blocks => new BigNumber(blocks).dividedBy('1008') // 1008 = blocks per week

export const weeksToBlocks = weeks => new BigNumber(weeks).times('1008') // 1008 = blocks per week
