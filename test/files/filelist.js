...
describe('file list', () => {
	it('renders a ul with the correct number of file and directory children', () => {
		const filelist = shallow(<FileList files={testFiles} showSearchField={false} path="" />)
	...
	it('renders a back button when path is set', () => {
		expect(shallow(<FileList files={testFiles} showSearchField={false} path="movies/" />).find('ul').children()).to.have.length(testFiles.size + 1)
	})
	it('navigates directories', () => {
		let filelist = shallow(<FileList files={testFiles} showSearchField={false} path="test1/test2/" actions={testActions} />)
		filelist.find('ul').children().first().simulate('click')
		expect(testActions.setPath.calledWith('test1/')).to.equal(true)

		filelist = shallow(<FileList files={testFiles} showSearchField={false} path="test1/" actions={testActions} />)
		filelist.find('ul').children().first().simulate('click')
