...
describe('file list', () => {
	...
	it('renders a ul with the correct number of file and directory children', () => {
		const filelist = shallow(<FileList hasPolled files={testFiles} selected={OrderedSet()} showSearchField={false} path="" />)
		expect(filelist.find('File')).to.have.length(testFiles.size)
	})
	it('renders a back button when path is set', () => {
		expect(shallow(<FileList hasPolled files={testFiles} showSearchField={false} selected={OrderedSet()} path="movies/" />).find('ul').children()).to.have.length(testFiles.size + 1)
	})
	describe('file selection', () => {
		it('selects files', () => {
			const filelist = shallow(<FileList hasPolled files={testFiles} showSearchField={false} selected={OrderedSet()} path="" actions={testActions} />)
		...
		it('deselects a file when selected and ctrl-clicked', () => {
			const filelist = shallow(<FileList hasPolled files={testFiles} showSearchField={false} selected={OrderedSet([testFiles.get(0), testFiles.get(1)])} path="" actions={testActions} />)
			const filenodes = filelist.find('File')
	...
	it('navigates directories', () => {
		let filelist = mount(<FileList hasPolled files={testFiles} selected={OrderedSet()} showSearchField={false} path="test1/test2/" actions={testActions} />)
		filelist.find('.back-button').first().simulate('click')
		expect(testActions.setPath.calledWith('test1/')).to.equal(true)

		filelist = mount(<FileList hasPolled files={testFiles} showSearchField={false} selected={OrderedSet()} path="test1/" actions={testActions} />)
		filelist.find('.back-button').first().simulate('click')
